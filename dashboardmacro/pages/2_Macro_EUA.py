...
def fallback_window_size(periodicidade: str) -> int:
    periodicidade = str(periodicidade).lower()
...
with row_b:
    range_behavior = st.selectbox("Sem dados no intervalo", RANGE_BEHAVIOR_OPTIONS, key="fred_range_behavior")
with row_c:
...
if show_table:
    if table_frames:
        ...
        table_df["valor"] = table_df["valor"].map(lambda value: format_br_number(value, CASAS))
